...
\end{equation*}
\begin{center}
  \includegraphics[width=0.5\linewidth]{Problem 1.png}
\end{center}

\begin{enumerate}
  \item 
  The first and third graphs are overfit and underfit respectively.
  The second graph is a better middle ground that makes good use of kernel regression's benefits,
  so I would probably use tau=50 for predictions.
  \item The MSE is minimized when tau is minimized, but this leads to overfitting. As tau approaches 0,
  the kernel function approaches $e^{-\infty}$, which is zero, meaning that we can effectively force the MSE to zero
